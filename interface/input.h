...
/*
NOTENOTE: This code uses a little hack. Since some special keys in Qt, the
windowing library that also is used to catch the input, have an additional bit
set, the key codes end up with numbers >= 255. In these cases, an attempt to
remove this bit is made and their value is tested again.  This might mean that
two keys could share the same internal key code, but this has not been tested.
*/
